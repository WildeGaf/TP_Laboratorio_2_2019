...
namespace Entidades
{
  public class Numero
  {
    ...
    {
      long binario, cantidad,exponente = 0;
      int i;
      string resultado;
      long suma = 0;
      cantidad = (long)binarioAux.Length;
      long.TryParse(binarioAux, out binario);
      for (i = (int)cantidad - 1; i >= 0; i--)
      {

        if (binarioAux[i] == '1')
        {
          suma += (long)Math.Pow(2, exponente);
        }
        exponente++;
      }
